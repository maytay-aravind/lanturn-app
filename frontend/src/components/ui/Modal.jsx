...
export function Modal({ open, onClose, title, children, size = 'md' }) {
  ...
  return (
    ...
    >
      <div className={`card w-full ${sizeClass} animate-scale-in shadow-2xl`}>
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <h2 className="text-base font-semibold text-brand-900">{title}</h2>
          <button
            onClick={onClose}
            className="h-8 w-8 rounded-lg flex items-center justify-center text-brand-400 hover:text-brand-600 hover:bg-slate-100 transition-colors"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
